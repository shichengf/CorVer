...
Each YAML pins the base model and extractor revisions. Prepare the corpus index and tokenizer from the public upstream resources below. Index/model weights are downloaded separately and keep their upstream licenses.

## Local corpus resources

Full training uses a local Wikipedia Infini-gram index and its matching Llama-2 tokenizer. Prepare both from the public upstream resources below.

1. Obtain the Llama-2 tokenizer from [Meta's official repository](https://huggingface.co/meta-llama/Llama-2-7b-hf), accepting its access terms when required. Download only the tokenizer; Llama-2 model weights are not needed. For example, after authenticating with Hugging Face:
   ...
   tokenizer.save_pretrained("resources/index_tokenizer")
   ```

2. Prepare English Wikipedia from the November 2023 snapshot (`20231101.en`), available through [wikimedia/wikipedia](https://huggingface.co/datasets/wikimedia/wikipedia). Follow the [official Infini-gram custom-index instructions](https://github.com/liujch1998/infini-gram/blob/master/docs/pkg_doc.md#indexing-custom-datasets) to build a local index. The builder accepts JSONL documents with a `text` field. Use the Llama-2 tokenizer (`--tokenizer llama`), version 4, `u16` tokens, and `--add_metadata` to produce the files expected by this entry point. Choose CPU, memory, and sharding settings for the available hardware; construction requires additional disk space for temporary files.

3. Set `CORVER_INDEX_DIR` and `CORVER_INDEX_TOKENIZER_PATH` to the resulting local directories, or pass `--index-dir` and `--index-tokenizer-path`. The index directory must contain `metadata.0`, `metaoff.0`, `offset.0`, `table.0`, and `tokenized.0`. The tokenizer directory must contain `tokenizer_config.json` and its vocabulary/model files. Training reads these directories locally.

Use the same tokenizer vocabulary and tokenization behavior for index construction and queries. Keep the corpus snapshot, text preprocessing, and document boundaries consistent.
