#!/bin/bash
# OLMo-2-13B-Instruct.
# Trains 100 steps, then evals step-100 on 5 closed-book QA datasets at max_tokens=1000.
set -u
set -o pipefail
...
EVAL_DIR="$TRAIN_DIR/eval_results/olmo2_13b"
mkdir -p "$EVAL_DIR" "$LOG_DIR"

# Required environment:
#   INFIGRAM_LOCAL_INDEX_DIR  path to a locally-built Llama-2-tokenized Wikipedia infini-gram index
# Optional:
#   STEPWISE_REWARD_MODE      full | judge_only | corver_only  (default: full)
...
RUN_DIR="$TRAIN_DIR/output/olmo2_13b"

# ---- Train 100 steps ----
echo ""
echo "############################################################"
echo "#  OLMo-2-13B (anti-loop training prompt)"
echo "############################################################"
cd "$TRAIN_DIR"
if [ -d "$RUN_DIR/checkpoint-100" ]; then
    echo "olmo2_13b already trained 100 steps"
else
    safe_run "olmo2_13b_grpo" python main_stepwise_olmo.py --config script/grpo_olmo2_13b.yaml
fi
cd "$PROJECT_ROOT"
...
for DS in "${DATASETS[@]}"; do
    ...
    OUT_NAME="olmo2_13b_step100_${DS_NAME}"
...
echo ""
echo "##  OLMo-2-13B train + step-100 eval DONE @ $(date)"
