#!/bin/bash
# Qwen3-4B CorVer launcher.
# Trains 100 GRPO steps + evals the step-100 checkpoint on 5 closed-book QA datasets at max_tokens=1000.
set -u
set -o pipefail
...
EVAL_DIR="$TRAIN_DIR/eval_results/qwen3_4b"
...
RUN_DIR="$TRAIN_DIR/output/qwen3_4b"
...
if [ -d "$RUN_DIR/checkpoint-100" ]; then
    echo "qwen3_4b already trained 100 steps"
else
    safe_run "qwen3_4b_grpo" python main_stepwise.py --config script/grpo_qwen3_4b.yaml
fi
cd "$PROJECT_ROOT"
