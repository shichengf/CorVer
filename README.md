...
[Training guide](docs/training.md) · [Baseline resources](#baseline-resources) · [MIT license](LICENSE)
...
For method details and experimental results, see the submitted manuscript.
...
# Prepare the corpus resources using docs/training.md.
export CORVER_INDEX_DIR="$PWD/resources/corpus_index"
export CORVER_INDEX_TOKENIZER_PATH="$PWD/resources/index_tokenizer"
...
Choose a YAML from the model table above; the same entry point accepts the 13B/14B configurations. Base-model and extractor revisions are pinned in each configuration. Prepare the corpus index and matching tokenizer using the [resource-preparation guide](docs/training.md#local-corpus-resources).
